...
var GlobalResponseSessionManager *ResponseSessionManager
...
func recaptchaProxyURL() string {
	if url := strings.TrimSpace(os.Getenv("RECAPTCHA_PROXY_URL")); url != "" {
		return url
	}

	// Render Blueprints can reference another service's private host:port,
	// but they cannot prefix it with "http://". Keep public/local configs on
	// RECAPTCHA_PROXY_URL and let Render use this helper variable.
	if hostport := strings.TrimSpace(os.Getenv("RECAPTCHA_PROXY_HOSTPORT")); hostport != "" {
...
func splitCookieConfig(cookieStr string) []string {
	cookieStr = strings.TrimSpace(cookieStr)
	if cookieStr == "" {
		return nil
	}

	// A full browser Cookie header often contains commas inside cookie values
	// (for example JSON-ish g_state payloads). Treat that as one cookie header
	// unless the config clearly provides multiple session_id entries.
	if strings.Contains(cookieStr, ";") && !strings.Contains(cookieStr, ",session_id=") && !strings.Contains(cookieStr, ", session_id=") {
...
func InitGSCookies() {
	...
	if cookieStr != "" {
		...
		for _, cookie := range splitCookieConfig(cookieStr) {
...
type RateLimitCookieEntry struct {
	Cookie         string    `json:"cookie"`
	ExpirationTime time.Time `json:"expiration_time"`
}

func ListRateLimitCookies() []RateLimitCookieEntry {
	var entries []RateLimitCookieEntry
	rateLimitCookies.Range(func(key, value any) bool {
		cookie, ok := key.(string)
		if !ok {
			return true
		}
		rl, ok := value.(RateLimitCookie)
		if !ok {
			return true
		}
		entries = append(entries, RateLimitCookieEntry{
			Cookie:         cookie,
			ExpirationTime: rl.ExpirationTime,
		})
		return true
	})
	return entries
...
func NewResponseSessionManager() *ResponseSessionManager {
	return &ResponseSessionManager{
		sessions: make(map[string]string),
	}
}

func (sm *ResponseSessionManager) AddSession(responseID string, projectID string) {
	if strings.TrimSpace(responseID) == "" || strings.TrimSpace(projectID) == "" {
		return
	}
	sm.mutex.Lock()
	defer sm.mutex.Unlock()
	sm.sessions[responseID] = projectID
}

func (sm *ResponseSessionManager) GetProjectID(responseID string) (string, bool) {
	sm.mutex.RLock()
	defer sm.mutex.RUnlock()
	projectID, exists := sm.sessions[responseID]
